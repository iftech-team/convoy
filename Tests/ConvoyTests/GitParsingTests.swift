...
@Test func filePreviewReadIsBounded() throws {
    let url = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
    defer { try? FileManager.default.removeItem(at: url) }
    try Data("small".utf8).write(to: url)
    #expect(TextFileInspector.readBounded(url) == Data("small".utf8))
    try Data(repeating: 65, count: TextFileInspector.maxBytes + 1).write(to: url)
    #expect(TextFileInspector.readBounded(url) == nil)
}
