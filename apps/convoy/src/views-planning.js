      ...
      ${planning.tasks
        ...
        <div class="row" data-task="${escape(task.id)}">
          ...
          <div class="row__body">
            ...
            <div class="row__meta">
              ${escape(task.status)} · ${escape(MODES[task.mode] ?? task.mode)}
...
export function activityView() {
  ...
  if (!events.length) {
    return empty("clock", "Nothing yet", "Start a session to see it here.");
  }
  const label = {
    started: "started",
    resumed: "resumed",
  ...
  return `
    <div class="section">
      <h2 class="section__title">Activity
        <span class="section__count">${events.length}</span>
      </h2>
      <span class="section__spacer"></span>
      ${button({ icon: "refresh", action: "refresh-activity", kind: "icon", title: "Refresh" })}
    </div>
    <p class="section__hint">Newest first. Kept to the last 200 events.</p>
    <div class="list">
      ${events
        .map(
          (event) => `
        <div class="row">
          <span class="row__mark">•</span>
          <div class="row__body">
            <div class="row__title">${escape(event.title)} · ${escape(label[event.kind] ?? event.kind)}</div>
            <div class="row__meta">${escape(when(event.at))} · ${escape(event.detail)}</div>
          </div>
        </div>`,
        )
        .join("")}
