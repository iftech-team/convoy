...
final class GitPanelModel: ObservableObject {
    ...
    func unstage(_ paths: [String]) { guard !paths.isEmpty else { return }; enqueue("Unstaging…") { [self] in _ = await git(["restore", "--staged", "--"] + paths) } }
    func stageAll() { enqueue("Staging…") { [self] in _ = await git(["add", "-A"]) } }
    func unstageAll() { enqueue("Unstaging…") { [self] in _ = await git(["reset", "-q"]) } }
