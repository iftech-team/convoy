...
final class GitInfoService: ObservableObject {
    ...
    nonisolated static func run(_ arguments: [String], in directory: String, timeout: TimeInterval = 8, input: Data? = nil, environment: [String: String] = [:]) -> (Int32, String)? {
        ...
        process.arguments = ["git", "--literal-pathspecs", "-C", directory, "--no-optional-locks"] + arguments
        ...
        // A Convoy launched from git/agent tooling must still operate on the chosen repository.
        ...
        // Drain stdout while feeding stdin; large patches must not deadlock against
        // git's diagnostic output before the watchdog can run.
        if let input {
            Thread.detachNewThread {
