...
2. **Task dependencies.** Let tasks depend on other tasks and run only when their prerequisites have been accepted. Detect dependency cycles and explain why a queued task is blocked.
...
4. **Preview parity: attention and layouts.** Bring the native macOS “Needs you” view to Windows/Linux, add acknowledgement or snooze, and restore the preview’s selected projects and split panes on reopen. Keep agent resume explicit.
