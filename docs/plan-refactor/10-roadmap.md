...
**Статус на 24.09.2026.** M1 закрыт: `convoy-core` собран, 61 тест зелёный,
`cargo clippy -- -D warnings` чист. M0 закрыт наполовину — каркас и прототип
написаны, но `vte4` не установлен, поэтому прототип ещё не запускался.
Ближайший шаг — `pacman -S vte4` и прогон `convoy-vte-probe` с живым Claude Code.
...
- [ ] **Запустить в нём настоящий Claude Code и Codex.** Проверить alt-screen,
...
**Выход:** рабочий прототип терминала. Если Claude Code в VTE выглядит плохо —
план пересматривается здесь, а не на пятой неделе.
