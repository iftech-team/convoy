...
## Отличия реализации от плана (24.09.2026)

**`codexLimits()` сделан не на `gio::Subprocess`.** План предлагал его из-за
двустороннего JSON-RPC и таймаута в 25 секунд. На практике протокол вынесен в
чистую функцию `provider::codex::handle_line`, которая тестируется без
процесса вообще, а ввод-вывод — обычный `std::process` в потоке через
`gio::spawn_blocking`. Это тот же механизм, что у всех остальных внешних
вызовов, и на один способ делать асинхронность меньше. `gio::Subprocess`
понадобился бы, если бы нужно было отменять запрос из UI — сейчас не нужно.

**Состояние агента показывается на вкладке.** `adw::TabPage::set_loading`
крутит спиннер, пока идёт ход, `set_indicator_icon` показывает, что агент ждёт
ввода или закончил. Заголовок вкладки несёт `★` для закреплённых и состояние
словом — то же, что в сайдбаре.
